...
pub fn render_music_panel(
    ...
    let name = player
        .current_song_name()
        .unwrap_or_else(|| "No Music".to_string());
    ...
    let list_hovered =
        rx_list >= w as i32 - (40.0 * scale) as i32 && ry_list < (35.0 * scale) as i32;
    ...
    let in_ctrl_row =
        my_i >= ctrl_y - (5.0 * scale) as i32 && my_i < ctrl_y + (25.0 * scale) as i32;

    // Mode Toggle
    let mode_hovered =
        in_ctrl_row && mx_i >= ctrl_start_x && mx_i < ctrl_start_x + (25.0 * scale) as i32;
    ...
    let mode_color = if mode_hovered {
        ui_primitives::apply_opacity(0xFFFFFF, opacity)
    } else {
        ui_primitives::apply_opacity(0x999999, opacity)
    };
    ui_primitives::draw_text_dw_ex(
        buffer,
        win_w,
    ...
    let prev_color = if prev_hovered {
        ui_primitives::apply_opacity(0xFFFFFF, opacity)
    } else {
    ...
    ui_primitives::draw_triangle(
        buffer,
        win_w,
        ctrl_start_x + (2.0 * scale) as i32,
        ctrl_y + (5.0 * scale) as i32,
        prev_w / 2,
        prev_h,
        prev_color,
        false,
        win_w,
        buffer.len() as u32 / win_w,
    );
    ui_primitives::draw_triangle(
        buffer,
        win_w,
        ctrl_start_x + (10.0 * scale) as i32,
        ctrl_y + (5.0 * scale) as i32,
        prev_w / 2,
        prev_h,
        prev_color,
    ...
    // Play/Pause
    let pp_hovered = in_ctrl_row
        && mx_i >= ctrl_start_x + btn_gap
        && mx_i < ctrl_start_x + btn_gap + (25.0 * scale) as i32;
    let pp_font_scale = if pp_hovered { 1.2 } else { 1.0 };
    let pp_color = if pp_hovered {
        ui_primitives::apply_opacity(0xFFFFFF, opacity)
    } else {
        ui_primitives::apply_opacity(0xEEEEEE, opacity)
    };
    ...
    if player.is_playing() {
        ...
        ui_primitives::draw_rounded_rect(
            buffer,
            win_w,
            ctrl_start_x + btn_gap + (2.0 * scale) as i32,
            ctrl_y + (4.0 * scale) as i32,
            pp_w / 3,
            pp_h,
            0,
            pp_color,
        ...
        ui_primitives::draw_rounded_rect(
            buffer,
            win_w,
            ctrl_start_x + btn_gap + (10.0 * scale) as i32,
            ctrl_y + (4.0 * scale) as i32,
            pp_w / 3,
            pp_h,
            0,
        ...
        ui_primitives::draw_triangle(
            buffer,
            win_w,
            ctrl_start_x + btn_gap + (4.0 * scale) as i32,
            ctrl_y + (4.0 * scale) as i32,
            pp_w,
            pp_h,
            pp_color,
    ...
    // Next
    let next_hovered = in_ctrl_row
        && mx_i >= ctrl_start_x + btn_gap * 2
        && mx_i < ctrl_start_x + btn_gap * 2 + (25.0 * scale) as i32;
    let next_font_scale = if next_hovered { 1.2 } else { 1.0 };
    let next_color = if next_hovered {
        ui_primitives::apply_opacity(0xFFFFFF, opacity)
    } else {
        ui_primitives::apply_opacity(0xCCCCCC, opacity)
    };
    ...
    ui_primitives::draw_triangle(
        buffer,
        win_w,
        ctrl_start_x + btn_gap * 2 + (5.0 * scale) as i32,
        ctrl_y + (5.0 * scale) as i32,
        next_w / 2,
        next_h,
        next_color,
    ...
    ui_primitives::draw_triangle(
        buffer,
        win_w,
        ctrl_start_x + btn_gap * 2 + (13.0 * scale) as i32,
        ctrl_y + (5.0 * scale) as i32,
        next_w / 2,
        next_h,
        next_color,
    ...
    let bar_h = if prog_hovered {
        (4.0 * scale).max(1.0) as u32
    } else {
        (2.0 * scale).max(1.0) as u32
    };
    let bar_y_offset = if prog_hovered {
        (2.0 * scale) as i32
    } else {
        (3.0 * scale) as i32
    };
    ...
    if player.list_visible && !songs.is_empty() {
        ...
        for i in start_idx..end_idx {
            ...
            let ry_f =
                (i as f32 * BASE_LIST_ITEM_HEIGHT as f32 - player.list_scroll_offset) * scale;
            let item_y = list_y + ry_f as i32;

            // Clip items outside of the list area with small buffer
            let clip_buffer = (2.0 * scale) as i32;
            if item_y < list_y - clip_buffer
                || item_y >= list_y + (visible_items as i32 * item_h) - clip_buffer
            {
                continue;
            }
            ...
            if is_current && item_text_w > item_max_w as f32 {
                ...
                item_scroll_x = if t < 1.0 {
                    0.0
                } else if t < 1.0 + (overflow / speed) as f64 {
                    -((t - 1.0) as f32 * speed)
                } else if t < 2.0 + (overflow / speed) as f64 {
                    -overflow
                } else {
                    -(overflow - (t - (2.0 + (overflow / speed) as f64)) as f32 * speed)
                };
            ...
            if is_current {
                target_font_size = if is_hovered {
                    12.0 * scale
                } else {
                    11.5 * scale
                };
        ...
        if songs.len() > max_visible_items {
            ...
            let total_content_h =
                (songs.len() as f32 * BASE_LIST_ITEM_HEIGHT as f32 * scale) as f32;
            ...
            let scroll_ratio = if max_scroll > 0.0 {
                player.list_scroll_offset / max_scroll
            } else {
                0.0
            };
...
pub fn check_music_panel_hit(
    ...
    if ry >= ctrl_y - (5.0 * scale) as i32 && ry < ctrl_y + (25.0 * scale) as i32 {
        ...
        } else if rx >= ctrl_start_x + btn_gap
            && rx < ctrl_start_x + btn_gap + (25.0 * scale) as i32
        {
            return Some(MusicPanelAction::Prev);
        } else if rx >= ctrl_start_x + btn_gap * 2
            && rx < ctrl_start_x + btn_gap * 2 + (25.0 * scale) as i32
        {
            return Some(MusicPanelAction::PlayPause);
        } else if rx >= ctrl_start_x + btn_gap * 3
            && rx < ctrl_start_x + btn_gap * 3 + (25.0 * scale) as i32
        {
    ...
    if player.list_visible && ry >= (BASE_PANEL_HEIGHT as f32 * scale) as i32 {
        ...
        let clicked_idx =
            ((player.list_scroll_offset * scale + list_ry as f32) / item_h_scaled).floor() as usize;
